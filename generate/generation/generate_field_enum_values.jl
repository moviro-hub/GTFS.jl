"""
    generate_field_enum_values(extracted_enums::Vector{FileEnumInfo}) -> Vector{String}

Generate source code for enum validation rules:
- `const ENUM_RULES` with per-file enum validation rules
"""
function generate_field_enum_values(extracted_enums::Vector{FileEnumInfo})
    lines = String[]
    # Header
    ...
    for file_enum_info in extracted_enums
        ...
        for enum_field in file_enum_info.fields
            push!(lines, "    (")
            push!(lines, "      field = \"$(enum_field.field)\",")
