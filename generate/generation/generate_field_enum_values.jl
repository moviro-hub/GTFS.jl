"""
    generate_field_enum_values(extracted_enums::Vector{EnumField}) -> Vector{String}

Generate source code for enum validation rules:
- `const ENUM_RULES` with per-field enum validation rules
"""
function generate_field_enum_values(extracted_enums::Vector{EnumField})
    lines = String[]
    # Header
    ...
    for enum_field in extracted_enums
        # Create composite key: "filename::fieldname"
        key = "$(enum_field.file)::$(enum_field.field)"
        push!(lines, "  \"$key\" => (")
