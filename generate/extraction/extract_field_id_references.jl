...
struct FileForeignInfo
    filename::String
    fields::Vector{FieldForeignInfo}  # Only fields with Foreign ID references
end
...
"""
function extract_all_field_id_references(file_defs::Vector{FileDefinition})
    result = FileForeignInfo[]

    for file_def in file_defs
        field_foreign_infos = FieldForeignInfo[]
        ...
        if !isempty(field_foreign_infos)
            file_info = FileForeignInfo(file_def.filename, field_foreign_infos)
