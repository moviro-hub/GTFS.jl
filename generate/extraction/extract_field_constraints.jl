...
struct FileFieldConstraintInfo
    filename::String
    fields::Vector{FieldConstraintInfo}  # Only fields with constraints
end
...
"""
    extract_all_field_constraints(file_defs::Vector{FileDefinition}, field_signs::Dict{String,String}) -> Vector{FileFieldConstraintInfo}
...
function extract_all_field_constraints(file_defs::Vector{FileDefinition}, field_signs::Dict{String,String})
    ...
    for file_def in file_defs
        field_constraint_infos = FieldConstraintInfo[]
        ...
        for (_, field_def) in iterate_all_fields([file_def])
            ...
            if constraint !== nothing
                field_info = FieldConstraintInfo(
                    field_def.fieldname,
                    constraint
                )
        ...
        if !isempty(field_constraint_infos)
            file_info = FileFieldConstraintInfo(file_def.filename, field_constraint_infos)
